...
class HighlightFragment : Fragment(), HighLightAdapterCallback {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (config!!.isNightMode) {
            mRootView!!.findViewById<View>(R.id.rv_highlights).setBackgroundColor(ContextCompat.getColor(activity!!,
                    R.color.black))
        }
        highlightsView.layoutManager = LinearLayoutManager(activity)
        highlightsView.addItemDecoration(DividerItemDecoration(activity, DividerItemDecoration.VERTICAL))
            ...
            if (!TextUtils.isEmpty(note)) {
                ...
                if (HighLightTable.updateHighlight(highlightImpl)) {
                    HighlightUtil.sendHighlightBroadcastEvent(
                            this@HighlightFragment.activity!!.applicationContext,
                            highlightImpl,
